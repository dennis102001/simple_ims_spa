<template>
  ...
  ">
    ...
    <!-- OVERVIEW -->
    <section class="p-2 min-h-fit w-full">
      <h4 class="mb-4 font-semibold">Dashboard Overview</h4>
      <div class="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-6 ">
        
        <div class="flex flex-col flex-1 p-6 lg:p-8 bg-white shadow-[0px_4px_10px_rgba(0,0,0,0.1)] rounded-xl hover-up-effect">
        ...
        <!-- QUICK ACTIONS -->
        <div class="flex flex-col justify-between flex-1 p-6 lg:p-8 bg-white shadow-[0px_4px_10px_rgba(0,0,0,0.1)] rounded-xl hover-up-effect">
          <p class="mb-2 font-bold">Quick Actions</p>
          <div class="flex flex-col flex-1 gap-3">
            <router-link :to="{ name: 'CreatePO' }" class="relative p-1 w-full overflow-hidden  transition-all transform duration-200 hover:shadow-[0px_2px_6px_rgba(0,0,0,0.04)] hover:bg-warmyellow-sec border border-gray-100 bg-warmyellow-pri rounded-lg group">
            ...
            <router-link :to="{ name: 'CreateSO' }" class="relative p-1 w-full overflow-hidden  transition-all transform duration-200 hover:shadow-[0px_2px_6px_rgba(0,0,0,0.04)] hover:bg-warmyellow-sec border border-gray-100 bg-warmyellow-pri rounded-lg group">
              <div class="relative z-10 flex flex-row items-center justify-start p-1 text-darkgray-sec">
                <div class="text-lg py-1 px-3 rounded-lg">
    ...
    <section class="p-2 ">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="w-full p-6 lg:p-8 bg-white shadow-[0px_4px_10px_rgba(0,0,0,0.1)] rounded-xl">
    ...
    <div class=" p-2 overflow-hidden grid grid-cols-1 lg:grid-cols-2 gap-6">
      <section class="flex flex-col flex-1 bg-white p-6 lg:p-8 overflow-hidden shadow-[0px_4px_10px_rgba(0,0,0,0.1)] rounded-xl transition-all transform">
        ...
        <div class="flex flex-col flex-1 overflow-hidden ">
          ...
          <div v-else-if="data.recentOrders && data.recentOrders.length > 0" class="overflow-auto" >
            ...
            <div v-for="po in data.recentOrders" class="flex flex-row flex-wrap w-full overflow-hidden border-b last:border-b-0 py-4">
              <div class="">
                <h6 class="font-semibold">{{ po.poNumber }}</h6>
                <div class="text-xs font-medium text-gray-600 ">{{ formatDate(po.orderDate) }}</div>
              </div>
              <div :class="['px-3 py-1 w-fit text-xs font-medium rounded text-nowrap ml-auto h-fit uppercase', 
        ...
        <div class="flex flex-col flex-1 overflow-hidden">
          ...
          <div v-else-if="data.recentSales && data.recentSales.length > 0" class="overflow-auto ">
            ...
            <div v-for="sale in data.recentSales" class="flex flex-row flex-wrap w-full overflow-hidden border-b last:border-b-0 py-4">
              <div class="">
                <h6 class="font-semibold">{{ sale.soNumber }}</h6>
                <div class="text-xs font-medium text-gray-500 ">{{ formatDate(sale.createdDate) }}</div>
              </div>
              <div :class="['px-3 py-1 w-fit text-xs font-medium rounded text-nowrap ml-auto h-fit uppercase', 
...
function getHomeData(){
  ...
  const startISO = startOfMonth.toISOString(); // "2025-12-01T00:00:00.000Z"
  const endISO   = endOfMonth.toISOString();   // "2025-12-31T23:59:59.000Z"
  
  axiosClient.get('/api/home-info', {params: {start: startISO, end: endISO}})
    .then(response => {
