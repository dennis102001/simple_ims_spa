<template>
    ...
    >
      <div v-if="inUse" class="modal absolute top-0 z-50 w-full h-full overflow-hidden bg-black/30 ">
            ...
            <div v-if="inUse" class="w-full max-w-sm mb-4 bg-white shadow-md  rounded-xl">
              <div class="flex flex-col items-center rounded-t-xl overflow-hidden">
                ...
                <div class="grid w-full grid-cols-2 gap-4 px-6 pt-2 pb-6 rounded-b-xl">
                  <ButtonRed @click="$emit('yes')" width="full">Delete</ButtonRed>
                  <ButtonWhite @click="$emit('cancel')" width="full">Cancel</ButtonWhite>
                </div>
              </div>
            </div>
          </transition>
