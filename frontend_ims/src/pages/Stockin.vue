<template>
    ...
    <div class="relative h-full mx-auto w-full max-w-[1000px] min-h-[500px] min-w-[200px]">
        ...
        >
            ...
            <div v-show="viewingItemsList" class="px-2 pt-2 pb-4 absolute h-full flex flex-col max-h-[800px] bg-white rounded shadow-[0_0_10px_rgba(0,0,0,0.1)] w-full">
                ...
                <div class="flex flex-col items-center mt-4 mb-10">
                    <h1 class="text-3xl font-semibold tracking-wide">Stock In</h1>
                </div>

                <div class="flex flex-col flex-1 overflow-hidden">
                    <!-- search and proceed -->
                    <div class="flex flex-col items-end justify-between px-2 mb-2 space-y-2 sm:space-y-0 sm:flex-row">
                        <ButtonYellow type="button" @click="showStockInForm" class="sm:order-2">
                    ...
                    <div class="flex-1 px-2 overflow-auto">
                        ...
                        <div v-if="!dataStatus" v-for="item in filteredItemsListData" class="grid grid-cols-[30%_20%_10%_10%_10%_10%_10%] body-row">
                            ...
                            <div class="justify-center body-data">
                                <button 
                                @click="toggleSelectItem(item.id, item.item_name, item.cost, item.price, item.unit)" 
                                :class=" selectedIds.includes(item.id) ? 'bg-red-500' : 'bg-gray-400' " class="px-4 py-2 text-white rounded">
        ...
        >
            <div v-show="viewingStockInForm" class="absolute w-full h-full">
                
                <form @submit.prevent="submitStockIn" class="mx-auto flex flex-col px-2 sm:px-4 pt-2 pb-4 bg-white rounded shadow-[0_0_10px_rgba(0,0,0,0.1)] max-h-[800px] h-full w-full max-w-2xl">
                    <div class="flex flex-col items-center mt-4 mb-10">
                        <h1 class="text-3xl font-semibold tracking-wide">Stock In Items</h1>
                    </div>
                        ...
                        <div class="flex-1 px-2 overflow-auto border-b-2">
                            ...
                            <div v-for="item in formData.selectedItemsList" class="grid grid-cols-[45%_15%_15%_15%_10%] body-row">
                                ...
                                <div class="justify-end body-data">
                                    <input v-model="item.quantityToAdd" type="text" class="w-full px-2 text-right rounded outline outline-1">
                                </div>
                                <div class="justify-center body-data">
                    ...
                    <div class="flex flex-row justify-between w-full px-2 space-x-4 sm:space-x-6 md:space-x-8">
                        ...
                        <ButtonRed @click="showItemsList('clear')" width="full" type="button">
                            CANCEL
                        </ButtonRed>
                    </div>
                </form>
...
<script setup>
import { useItemsListTransition, useFormTransition, useAlertLoadingTransition } from '../composables/useTransition';
...
const viewingStockInForm = ref(null)
...
function submitStockIn(){
    ...
    axiosClient.post('/api/stock-in', formData.value)
        .then(response => {
            loading.value = false
...
function showItemsList(clear) {
    ...
    viewingStockInForm.value = false
...
function toggleSelectItem(id, name, cost, price, unit){
